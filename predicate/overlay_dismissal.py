"""
Overlay Dismissal Utilities for proactive blocking overlay removal.
...
The SDK's built-in ModalDismissalConfig (in planner_executor_agent.py) triggers
AFTER DOM changes from actions, but many sites show blocking overlays immediately
on page load. This module handles those initial blocking overlays.
...
Usage:
    ...
    result = await dismiss_overlays_before_agent(runtime, browser)
    print(f"Dismissed {result.overlays_before - result.overlays_after} overlays")
"""
...
if TYPE_CHECKING:
    ...
    from .browser import AsyncPredicateBrowser
...
def _label_variants(el: Any) -> list[str]:
    ...
    try:
        ...
        out.append(
            _norm(getattr(el, "aria_label", None) or getattr(el, "ariaLabel", None) or "")
        )
...
async def dismiss_overlays(
    ...
    *,
    ...
    Example:
        ...
        from predicate.overlay_dismissal import dismiss_overlays

        async with AsyncPredicateBrowser() as browser:
            await browser.goto("https://example.com")
            runtime = AgentRuntime(backend=browser.backend)

            result = await dismiss_overlays(runtime, browser, verbose=True)
    ...
    # Snapshot options
    snap_kwargs: dict[str, Any] = {"limit": snapshot_limit}
    ...
    if verbose:
        logger.info(
            f"[OVERLAY] Done: status={status}, before={overlays_before}, after={overlays_after}"
        )
...
async def dismiss_overlays_before_agent(
    ...
    *,
    ...
    Example:
        ...
        from predicate.browser import AsyncPredicateBrowser
        from predicate.overlay_dismissal import dismiss_overlays_before_agent
